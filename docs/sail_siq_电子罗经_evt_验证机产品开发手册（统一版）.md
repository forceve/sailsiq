...
- Fleet 起航：倒计时 + 线位 + Burn/Late
- Match 起航：**4 分钟进 Box 区 + 航速参考（Dial‑up / 冲刺）+ Burn/Late**
- 航段：迎风/顺风对标（进标阶段）
...
- Match：**Box 起航流程（4min）+ 航速显示（Dial‑up / 冲刺）+ Burn/Late**
...
> 说明：EVT **不以“监控对手态势”为核心假设**。标准 Match 起航中，设备的价值在于 **时间管理 + 航速控制**，而不是实时感知对手。

#### 8.3.1 起航流程假设（标准 Match）

- **T‑4:00**：进入 Box 区（Entry）
- **T‑2:00 \~ T‑1:00**：Dial‑up（控制航速、保持可操作性）
...
- 倒计时（T‑mm\:ss）
- 倒计时秒级校正：比赛中允许 ±1s 微调
- **当前 SOG（大号显示）** —— 用于 Dial‑up / 冲刺
- 到线垂距（signed distance）
- **Burn / Late（与 Fleet 启航逻辑一致）**

> Match 起航中，Burn/Late 同样用于判断“是否需要继续烧时间”或“是否必须加速”。

#### 8.3.3 Box 区逻辑（EVT 简化版）

- Box 区作为**软约束**：
  - 设备提示是否已进入 Box（基于到线距离阈值）
  - **不强制绘制 Box 边界**（避免复杂规则差异）
- 进入 Box 后，UI 自动切换为 Match Start 页面（建议）。
...
1. Compass（默认）
2. Start（Fleet/Match）
3. Course/Mark（Upwind/Downwind mark）
4. Finish（Upwind/Downwind finish）
...
- **WAIT（等待）**：赛前待机/赛后冷却，显示时间/电量/定位状态
- **START\_ACTIVE（启航倒计时）**：Fleet/Match 起航页面
...
- 进入等待页面（默认上电即 WAIT）
- **按下计时**（ACTION）后：进入 Start 倒计时（Fleet 或 Match）
...
- 通过后进入 **WAIT 冷却计时**：`X min`（用户可设，EVT 默认 2min）
  - 目的：避免在终点附近回航/转圈导致反复触发
- 冷却结束后：回到 WAIT，并允许再次按 ACTION 开启下一次计时
...
- `EVENT_START_TIMER`
- `EVENT_START_CROSS`
- `EVENT_MARK_ROUND (WM/OF/LM, side, dtg_min)`
- `EVENT_FINISH_CROSS`
- `EVENT_WAIT_ENTER/EXIT`

---

## 10A. 按钮与交互逻辑总表（四键 · 无长按 · 支持组合键）

> 设计原则：**四个物理按钮，仅支持短按；允许同时按下形成组合键；行为与当前场景强相关，避免模式爆炸。**
...
- **仅短按**：不支持长按
- **支持组合键**：同一时间窗（例如 150ms 内）识别为组合
- **组合键优先级**：组合键 > 单键
- **防误触**：对“破坏性动作”必须使用组合键（例如 Reset/清除）
...
- PAGE 永远不改变数据，只改变“看什么”
- ACTION 永远是“我现在最可能想做的事”
- ADJUST 仅在“可调页面”生效（倒计时、设置、字段切换）
- 所有关键设置（赛场标定、清除、导出）集中在 Settings

### 10A.4 全局组合键（Global Chords）

| 组合键                 | 功能                 | 说明                |
| ------------------- | ------------------ | ----------------- |
| PAGE + ACTION       | 进入/退出 Settings（快捷） | 比反复翻页更快           |
| ADJUST − + ADJUST + | 屏幕全刷/强制重绘          | 处理残影/异常显示         |
| PAGE + ADJUST −     | 上一页面（反向翻页）         | 解决只能单向翻页的问题       |
| PAGE + ADJUST +     | 下一页面（等同 PAGE）      | 保持一致性             |
| ACTION + ADJUST −   | “撤销/返回”            | 仅在 Settings/编辑时启用 |
| ACTION + ADJUST +   | “确认/保存”            | 仅在 Settings/编辑时启用 |

> 注：全局组合键在比赛中也可用，但 **PAGE+ACTION（进设置）** 建议加二次确认提示（1 秒内再次按）以防误入。

### 10A.5 各页面按钮行为一览（Single Key + Chords）
...
| ACTION       | 快捷信息切换（预留）   |
| ADJUST − / + | 无            |
| PAGE+ACTION  | 进入 Settings  |

#### B) Start – Fleet

| 输入                | 行为                                   |
| ----------------- | ------------------------------------ |
| PAGE              | 切换到下一个页面（Mark / Finish / Compass 循环） |
| ACTION            | SYNC 倒计时到整分钟                         |
| ADJUST −          | 倒计时 −1min（5→3→1）                     |
| ADJUST +          | 倒计时 +1min（1→3→5）                     |
| ACTION + ADJUST + | 一键重置倒计时到预设（默认 5:00）                  |
| ACTION + ADJUST − | 切换预设组（5/3/1 vs 自定义）                  |

#### C) Start – Match（标准 Box 起航）

| 输入                | 行为                                    |
| ----------------- | ------------------------------------- |
| PAGE              | 切换到下一个页面（Mark / Finish / Compass 循环）  |
| ACTION            | Box Entry 触发：开始/重置 4:00 计时（一次按下）      |
| ADJUST − / +      | 无（避免误操作）                              |
| ACTION + ADJUST + | 一键重置到 4:00（需要）                        |
| ACTION + ADJUST − | 切换 Match 预设（4:00/3:00 可选，EVT 默认 4:00） |

> Match 页面必须大号显示 **SOG**；Burn/Late 与到线垂距持续显示。

#### D) Upwind / Downwind Mark（对标）

| 输入                | 行为                             |
| ----------------- | ------------------------------ |
| PAGE              | 切换到 Finish 页面                  |
| ACTION            | 确认已绕标 → 切换到下一个航段（WM→LM→Finish） |
| ADJUST −          | 循环切换次要字段（DTG / VMG / ETA）      |
| ADJUST +          | 循环切换次要字段（DTG / VMG / ETA）      |
| ACTION + ADJUST + | 标记“绕标完成”并写日志事件（强制）             |

#### E) Finish（Upwind / Downwind Finish）

| 输入                | 行为                  |
| ----------------- | ------------------- |
| PAGE              | 切换到 Compass         |
| ACTION            | 标记完成（Finish logged） |
| ADJUST − / +      | 无                   |
| ACTION + ADJUST + | 结束 Session（停止记录，可选） |

#### F) Settings / Calibration

| 输入                | 行为                        |
| ----------------- | ------------------------- |
| PAGE              | 切换设置子页                    |
| ACTION            | 执行当前高亮操作（开始/停止采样、开始/停止拟合） |
| ADJUST −          | 数值 − / 选项向下               |
| ADJUST +          | 数值 + / 选项向上               |
| ACTION + ADJUST + | 确认/保存（用于关键写入）             |
| ACTION + ADJUST − | 取消/返回                     |

### 10A.6 赛场标定相关映射（Settings 内）

| 功能                 | 进入方式                                 | 触发               |
| ------------------ | ------------------------------------ | ---------------- |
| Set Pin            | Settings → Calibration → Start Line  | ACTION 开始/结束采样   |
| Set Boat           | Settings → Calibration → Start Line  | ACTION 开始/结束采样   |
| Fit Line           | Settings → Calibration → Start Line  | ACTION 开始/结束拟合   |
| Set Mark (Quick)   | Settings → Calibration → Marks       | ACTION 开始/结束采样   |
| Orbit Mark         | Settings → Calibration → Marks       | ACTION 开始/结束绕标记录 |
| Set Finish (FP/FB) | Settings → Calibration → Finish Line | ACTION 开始/结束采样   |

### 10A.7 设计说明（Why this works）

- 四键无长按 → 显著降低水上误操作
- 组合键用于“高风险动作”（Reset/保存/结束 session）→ 更安全
- Fleet 与 Match 起航共享同一套几何内核（到线/TTL/Burn）但 UI 强调不同
- 标定与比赛操作彻底分离：比赛只看/同步/确认，标定在 Settings 做完
